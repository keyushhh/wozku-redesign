...
export default function EditorialHero({ onOpenDemo }: { onOpenDemo: () => void }) {
  ...
  return (
    <section className="relative z-10 min-h-[640px] overflow-hidden pt-9 lg:pt-10 flex flex-col justify-start gap-12 lg:gap-16">
      <div className="pointer-events-none absolute left-1/2 top-0 h-[235px] w-[760px] -translate-x-1/2 rounded-full bg-primary-500/[0.06] blur-[100px]" />
      <div className="relative mx-auto max-w-[1600px] px-6 text-center">
        <motion.h1 {...rise(0.06)} className="mx-auto mt-2 max-w-4xl 2xl:max-w-5xl font-display text-[40px] font-black leading-[0.98] tracking-[-0.055em] text-neutral-950 sm:text-5xl lg:text-[56px] 2xl:text-[72px]">Your story travels further<br /><span className="text-primary-650">when people share it.</span></motion.h1>
        <motion.p {...rise(0.12)} className="mx-auto mt-2 max-w-xl 2xl:max-w-2xl text-[13px] 2xl:text-base leading-relaxed text-neutral-500">Help the people closest to your brand share with confidence—and see the impact of every conversation.</motion.p>
      ...
      <motion.div {...rise(0.25)} className="relative mx-auto mt-auto grid h-[384px] max-w-[1440px] w-full grid-cols-[1.02fr_1.12fr_.98fr_1.12fr_1.02fr] items-end gap-3 px-4 sm:gap-4 sm:px-6">
        ...
        <FolderCardB surface="var(--primary-950)" className="h-[294px]">
          <div className="flex h-full flex-col justify-between p-5 text-white"><span className="text-[10px] font-medium text-white/65"></span><div><div className="flex -space-x-2"><span className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-[var(--primary-950)] bg-rose-400 text-[8px] font-bold">AM</span><span className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-[var(--primary-950)] bg-sky-400 text-[8px] font-bold">RK</span><span className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-[var(--primary-950)] bg-amber-400 text-[8px] font-bold">NL</span></div><p className="mt-3 font-display text-2xl font-black leading-[0.95]">Make every share feel personal.</p><p className="mt-2 max-w-[165px] text-[8px] leading-relaxed text-white/60">A clear voice for the networks that trust it.</p></div><button onClick={onOpenDemo} className="flex items-center justify-between border-t border-white/10 pt-2.5 text-[9px] font-bold text-[var(--accent-300)]"><span>Explore advocacy</span><ArrowUpRight className="h-3.5 w-3.5" /></button></div>
        </FolderCardB>

        <FolderCardD surface="#dbe5e5" className="h-[222px]">
          <div className="flex h-full flex-col justify-end p-5 text-left"><div className="mb-auto flex h-8 w-8 items-center justify-center rounded-full bg-white/80 text-primary-650"><Megaphone className="h-4 w-4" /></div><p className="font-display text-xl font-black leading-[0.98] text-neutral-900">Start with<br />one story.</p><p className="mt-1.5 text-[8px] text-neutral-600">Bring your next launch to the people who matter.</p><div className="mt-3 flex items-center gap-2 rounded-xl bg-white/80 p-1.5"><input aria-label="Work email" type="email" placeholder="Enter work email" className="min-w-0 flex-1 bg-transparent px-2 text-[9px] text-neutral-800 outline-none placeholder:text-neutral-500" /><button onClick={onOpenDemo} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-neutral-950 text-white"><ArrowUpRight className="h-3.5 w-3.5" /></button></div></div>
        </FolderCardD>

        <FolderCardC surface="var(--primary-950)" className="h-[294px]">
