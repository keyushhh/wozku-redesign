...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className={`w-full rounded-xl border border-neutral-200 bg-white px-3.5 py-2.5 text-xs text-neutral-900 placeholder:text-neutral-400 transition-all focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100/60 focus:outline-hidden ${props.className ?? ''}`} />;
}

export function Button({ className = '', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button {...props} className={`rounded-xl bg-indigo-600 px-4 py-3 text-xs font-bold text-white transition-colors hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60 ${className}`} />;
}
...
export function CustomSelect({ label, options, value, onChange, tone = 'light', open, onOpenChange }: { label: string; options: string[]; value: string; onChange: (value: string) => void; tone?: 'light' | 'dark'; open?: boolean; onOpenChange?: (open: boolean) => void }) {
  ...
  return (
    <div className="relative w-full text-left" ref={dropdownRef}>
      ...
      <button type="button" onClick={() => setOpen(!isOpen)} className={`flex w-full items-center justify-between rounded-xl px-3.5 py-2.5 text-xs transition-colors focus:outline-none ${tone === 'light' ? 'border border-neutral-200 bg-white text-neutral-800 hover:border-neutral-300 focus:border-indigo-500' : 'border border-fixed-white/10 bg-[#141418] text-fixed-white hover:border-fixed-white/20 focus:border-indigo-550'}`}>
